...
class Donator(commands.Cog):
    ...
    async def setcolor(self, ctx, *, colorHex : str):
        '''>setcolor <color>'''
        donatorRole = discord.utils.find(lambda r: r.name == 'Donators', ctx.message.author.roles)
        if donatorRole == None:
            await self.bot.say(f"{ctx.message.author.name}, You're not an active patron of Miki. You can be one by donating monthly on <https://www.patreon.com/mikibot>")
            return
        
        existRole = discord.utils.find(lambda r: r.name.upper() == '#' + colorHex.upper().replace(' ','').replace('#',''), ctx.message.server.roles)
        if existRole != None:
            await self.bot.say(f"{existRole.name} given to {ctx.message.author.name}")
